...
| 资源管理器优化 | 关闭云广告、显示受保护系统文件 |
| 始终显示文件扩展名 | 强制显示扩展名，降低伪装脚本风险 |
| 禁用保留存储 | 释放 Windows 预留磁盘空间 |
...
| 功能 | 说明 |
|------|------|
| 经典右键菜单 | 恢复完整经典菜单，免去「显示更多选项」 |
| 任务栏左对齐 | 任务栏图标左对齐（经典布局） |
| 任务栏结束任务 | 任务栏右键直接结束进程（23H2+） |
| 隐藏 Gallery | 隐藏资源管理器导航栏图库入口 |
| 隐藏 Home | 隐藏资源管理器导航栏主页入口 |
| 禁用 Snap 悬停提示 | 关闭最大化按钮 Snap Layouts 弹出提示 |
| 关闭开始菜单推荐广告 | 关闭开始菜单推荐区应用/技巧广告 |
| 文件夹悬停详细信息 | 悬停文件夹时显示大小与内容摘要 |
| 禁用搜索高亮 | 关闭任务栏搜索框动态高亮内容 |
| 任务栏时钟显示秒 | 任务栏时钟精确到秒 |
| 隐藏 OneDrive 导航栏 | 隐藏侧栏 OneDrive（不停同步） |
| 禁用锁屏 Spotlight | 关闭锁屏动态壁纸与趣味提示 |
| 标题栏显示完整路径 | 资源管理器标题栏显示完整路径 |
| 启用 Win32 长路径 | 突破 MAX_PATH 260 限制 |
...
本产品通过 WuGuard 飞书多维表格做用户端授权校验（直连飞书只读 OpenAPI，不依赖管理员本机 `127.0.0.1:8090`）。
